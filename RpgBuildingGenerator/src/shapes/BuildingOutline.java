...
public class BuildingOutline extends Shape {
    ...
    public void addPoint(Vector2f point) {
        ...
        if (points.size() == 0 || this.isActive()) {
            points.add(point);
    ...
    private boolean isComplete() {
        ...
        // Detects if last line segment ends at the start point   
        if (points.size() <= 2){
            complete = false;
        } else if (points.get(0) == points.get(points.size() -1)) {
            complete = true;
        } else
            complete = false;
        System.out.println("Building outline complete=" + complete);      
        return complete;       
    }
